...
We can interpret the covariance matrix as a quadratic form (i.e., a quadratic function, mapping a vector to a scalar), $Q(\mathbf{v})=\mathbf{v}Y^\mathsf{T}T\mathbf{v}$, that tells us, when given a unit vector $\mathbf{v}$, how much of the data's
