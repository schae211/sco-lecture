...
plot( x, y )
lines( xg, sapply( xg, local_regression_adaptive_smooth, x, y, hn=30 ), col="red" )
lines( xg, true_function( xg ), col="blue" )
```
...
plot( xg, sapply( xg, kernel_smooth_Poisson, x, k, s, h=.1 ), 
  type="l", col="red", log="y" )

lines( xg, true_fraction( xg ), col="blue" )

```
#### Average of Poisson-distributed variables
...
```{r}
local_regression_smooth_Poisson <- function( x0, x, k, s, h ) {
  w <- tricube( ( x - x0 ) / h )
  fit <- glm( k ~ x, weights=w, offset=log(s), family=poisson("log") )
  unname( coef(fit)[1] + x0 * coef(fit)[2] )
}
```
...
```{r}
yhat <- sapply( xg, local_regression_smooth_Poisson, x, k, s, .1 )
