...
tibble(x=seq( 0, 1, length.out=1000 )) %>%
  dplyr::mutate(y=true_function(x)) %>%
  ggplot() +
...
n <- 100
set.seed( 13245769 )
df <- tibble::tibble(x = runif(n)) %>%
  dplyr::mutate(y = true_function(x) + rnorm(n, sd=.1))
ggplot(df) +
...
plot( 
  xg, 
  sapply( xg, kernel_smooth, df$x, df$y, h=.1), 
...
plot( 
  xg, 
  sapply( xg, local_regression_smooth, df$x, df$y, h=.1), 
  type="l", col="red", ylim=c( -.1, 1.5 ) )

lines( xg, true_function( xg ), col="blue" )
```

Here, this did not make too much difference, but sometimes, switching from simple kernel smoothing to local regression helps a lot.
...
plot( 
  xg, 
  sapply( xg, local_quadratic_regression_smooth, df$x, df$y, h=.1), 
  type="l", col="red", ylim=c( -.1, 1.5 ) )
...
```{r}
plot( df$x, df$y, xlim=c(0,1), ylim=c(-.2,1.6) )

set.seed(42)
for( i in 1:100 ) {
  bss <- sample.int( length(df$x), replace=TRUE )
  lines( xg, sapply( xg, kernel_smooth, df$x[bss], df$y[bss], h=.1 ), 
...
for( i in 1:100 ) {
  bss <- sample.int( length(df$x), replace=TRUE )
  lines( xg, sapply( xg, local_quadratic_regression_smooth, df$x[bss], df$y[bss], h=.1 ), 
     col = adjustcolor( "red", alpha=.1 ) ) }
...
Calculate for each data point its y value, using the y values of its neighbors, but the point's own y value.

```{r}
yhat_loo <- sapply( 1:length(df$x), function(i)
  kernel_smooth( df$x[i], df$x[-i], df$y[-i], h=.1 ) ) 

plot( df$x, df$y )
points( df$x, yhat_loo, col="red" )
```

Now we can calculate a mean squared error (MSE):

```{r}
mean( ( df$y - yhat_loo )^2 )
```
...
```{r}
yhat_loo <- sapply( 1:length(df$x), function(i)
  local_quadratic_regression_smooth( df$x[i], df$x[-i], df$y[-i], h=.1 ) ) 
...
-> The scaling factor basically determines how much the 0 can pull down the curve!

```{r}
plot( x, log( k/s * 1e3 + 1 ) )
plot( x, log( k/s * 1e4 + 1 ) )
plot( x, log( k/s * 1e5 + 1 ) )
```
...
Note that we only need to know the variance but not the likelihood, because here we weight with the inverse variance -> but what if the observed count is 0? -> not an issue because the GLM uses the predicted values to predict the variance

```{r}
# check what information we have about the distribution
str(poisson())
```
...
TODO: But what about using an adaptive bandwidth here!?
